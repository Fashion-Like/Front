...
const PostForm = ({ setIsOpenModal, prevPost, isEdit, setIsEdit }) => {
	...
	const [tag, setTag] = useState(
		!isEdit ? 'Selecciona' : postToUpdate.tags[postToUpdate.tags.length - 1],
	);
	...
	const onSubmit = (e) => {
		...
		if (!isEdit) {
			...
			updatePost(postToUpdate.id, updatedPost).then((response) => {
	...
	return (
		<div>
			<HeaderPost>
				...
				<Select name="tag" value={tag} onChange={handleTag} options={TAGS}>
					{TAGS.map((option) => (
